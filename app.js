...
var budgetController = (function () {
  ...
  var data = {
    ...
    budget: 0,
    percentage: -1, // because it does not exist, not 0.
  };
  ...
  return {
    ...
    calculateBudget: function () {
      ...
      // 3. Calculate the % of the inc spent.
...
function saveStaticDataToFile() {
  var blob = new Blob(
    [JSON.stringify(JSON.parse(localStorage.getItem("localBudget")))],
    { type: "text/plain;charset=utf-8" }
  );
  saveAs(blob, "localBudget.txt");
}
...
function readFileAsString() {
  ...
  reader.onload = function (event) {
    localStorage.setItem("localBudget", event.target.result);
    location.reload();
  };
  reader.readAsText(files[0]);
}
function activateInput() {
  click(document.getElementById("upload"));
}
